...
Nexus AI Code (`nexus-ai-code` v1.0.0-beta) is a VS Code extension and CLI-oriented AI coding cockpit. It routes user prompts to installed CLI coding agents, streams output into a React webview, persists workspace chat history, supports provider detection, agent capability routing, pipeline pre-steps, debug/review modes, MCP-assisted context, subagents, and workflow-agent templates.
