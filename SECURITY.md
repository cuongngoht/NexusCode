...
- **MCP tool approval**: High-risk MCP tool calls (file writes, shell commands, network access, destructive operations) require user approval by default (`nexus.mcp.requireApprovalForHighRiskTools` is `true` by default). MCP is disabled by default (`nexus.mcp.enabled` is `false` by default). Custom MCP servers default to `risk: "high"`, so they are gated unless you lower that level yourself. `risk: "medium"` always prompts, regardless of `requireApprovalForHighRiskTools`; `risk: "low"` never prompts, so set it only for servers you fully trust. Approval is deny-by-default: a request that is not answered within `nexus.mcp.approvalTimeoutMs` (default 120s) is rejected and the tool does not run.
- **MCP credentials are stored in the workspace**: `mcp.customServers[*].headers`, `mcp.customServers[*].env` and `mcp.presets.context7.apiKey` are written to `.nexus/config.json` in plaintext (the Settings panel masks them in the UI, but the file on disk is not encrypted). Keep `.nexus/` out of version control if you store tokens there.
